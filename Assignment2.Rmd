...
The following code will call the libraries and data, and put the means, standard deviations, 95% confidence intervals, and interquartile ranges for each of the continuous variables in one table.
...
summary_stats_contvar<- data.frame(c(variables,mean,standard_deviation,low_CI_95,high_CI_95,IQ_Range))
colnames(summary_stats_contvar)<- c("variable","mean","standard_deviation","low_CI_95","high_CI_95","IQ_Range")
rownames(summary_stats_contvar)<- summary_stats_contvar$variable
summary_stats_contvar<- summary_stats_contvar[,2:6]
```
...
paste("The average household income earned per month in our sample of MA residents is approximately $", prettyNum(summary_stats_contvar[2,1], digits = 2, big.mark = ","), ". Half of the MA residents are in households that earn more than $", prettyNum(median(person_data$household_income), digits = 2, big.mark = ","), "per month, and half of them are in households that earn less than that. We can say with 95% confidence that the true mean household income per month for the population of MA residents is between $", prettyNum(summary_stats_contvar[2,3], digits = 3, big.mark = ","), "and $", prettyNum(summary_stats_contvar[2,4], digits = 3, big.mark = ","), ". The distribution appears to be skewed right. This makes sense because the sample of MA residents here excludes anyone who does not have a positive income, whereas there is no cap on the highest income included.")
